...
$('#analysis-settings').onclick=()=>{
  ...
  dialog('分析设置',`<p class="dialog-description">选择下一次单个或批量分析的转录方式。不会改变正在运行的任务，也不会自动覆盖已有转录。</p><label for="analysis-engine">转录方式</label><select id="analysis-engine" class="export-mode"><option value="local" ${analysisEngine==='local'?'selected':''}>本地转录 · 数据不上传</option><option value="dashscope" ${analysisEngine==='dashscope'?'selected':''}>阿里云百炼 · paraformer-v2</option></select><div id="local-analysis-settings"><label for="analysis-model">本地转录模型</label><select id="analysis-model" class="export-mode">${Object.entries(analysisModels).map(([key,name])=>`<option value="${key}" ${key===analysisModel?'selected':''}>${name}${key==='large-v3'?' · 默认高精度配置':''}</option>`).join('')}</select><label for="analysis-concurrency">本地同时分析数量</label><select id="analysis-concurrency" class="export-mode">${[1,2,3].map(n=>`<option value="${n}" ${n===(state.status.analysis_concurrency||2)?'selected':''}>${n} 个任务</option>`).join('')}</select><div id="model-download-panel" aria-live="polite"></div></div><div id="cloud-analysis-settings"><div class="cloud-status ${cloud.configured?'ready':'missing'}"><strong>${cloud.configured?'云端配置已就绪':'云端配置未完成'}</strong><span>固定模型：paraformer-v2</span>${cloud.error?`<p>${esc(cloud.error)}</p>`:missing?`<p>缺少环境变量：${esc(missing)}</p>`:'<p>凭证只从环境变量读取，不会发送到前端。</p>'}</div><label for="cloud-concurrency">云端同时分析数量</label><input id="cloud-concurrency" class="export-mode" type="number" min="1" max="50" step="1" value="${state.status.cloud_concurrency||30}"><p class="dialog-description">支持 1–50 个任务并行。每个媒体会临时上传至你配置的 OSS，生成 72 小时签名地址；转写结束后自动删除临时对象。</p></div><label for="export-concurrency">同时导出数量</label><select id="export-concurrency" class="export-mode">${[1,2,3].map(n=>`<option value="${n}" ${n===(state.status.export_concurrency||2)?'selected':''}>${n} 个视频</option>`).join('')}</select><p class="dialog-description">导出会使用本机处理器。建议使用 2 个；电脑明显卡顿时改为 1 个。</p>`,'保存设置',async()=>{
    ...
    await api('/api/settings',{analysis_concurrency:Number($('#analysis-concurrency').value),cloud_concurrency:cloudConcurrency,export_concurrency:Number($('#export-concurrency').value)});await poll();analysisModel=model;analysisEngine=engine;try{localStorage.setItem('autocut:model',model);localStorage.setItem('autocut:engine',engine);}catch{}updateModelSetting();updateControls();toast(engine==='dashscope'?'下一次分析使用阿里云百炼 paraformer-v2':'下一次分析使用本地 '+analysisModels[model]);
  });
...
function batchChoices(files, selected, quickUnanalyzed=false, rangeSelect=false) {const digits=String(Math.max(files.length,1)).length,listMax=Math.max(files.length,1);return `<div class="batch-controls"><button id="select-all" class="quiet">全选</button>${rangeSelect?`<div class="range-select" role="group" aria-label="按序号选择导出范围"><label>第 <input id="range-start" type="number" min="1" max="${listMax}" value="1" inputmode="numeric" aria-label="起始序号"> 个</label><span>到</span><label>第 <input id="range-end" type="number" min="1" max="${listMax}" value="${Math.min(200,listMax)}" inputmode="numeric" aria-label="结束序号"> 个</label><button id="select-range" class="quiet primary">选择范围</button></div>`:''}${quickUnanalyzed?'<button id="select-next-200" class="quiet primary">选择最近未分析的 200 个</button>':''}<button id="select-none" class="quiet">清空</button><span id="batch-selection-count" class="small" aria-live="polite"></span></div><div class="batch-list">${files.map((f,index)=>`<label class="batch-row"><input type="checkbox" value="${f.id}" data-analyzed="${f.analyzed?'true':'false'}" ${selected(f)?'checked':''}>${rangeSelect?`<span class="batch-index" aria-label="第 ${index+1} 个">${String(index+1).padStart(digits,'0')}</span>`:''}<span>${esc(f.name)}</span><small>${f.analyzed?'已分析':'未分析'}</small></label>`).join('')}</div>`;}
function wireChoices() {
  ...
  if($('#select-range'))$('#select-range').onclick=()=>{
    const all=inputs(),start=Number($('#range-start').value),end=Number($('#range-end').value);
    if(!Number.isInteger(start)||!Number.isInteger(end)||start<1||end<start||end>all.length){toast(`请输入 1 到 ${all.length} 之间的有效起止序号`);return;}
    if(end-start+1>200){toast('单次最多选择 200 个，请缩小序号范围');return;}
    all.forEach((x,index)=>x.checked=index>=start-1&&index<end);updateCount();
    toast(`已选择第 ${start} 到第 ${end} 个视频，共 ${end-start+1} 个`);
  };
