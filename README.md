...
- **200 个一批快速选择**：批量分析窗口可按当前素材顺序，一键选择最近的 200 个未分析视频，并自动跳过已分析项；剩余不足 200 个时会全部选中。
...
分析设置支持同时分析 1–3 个视频、同时导出 1–3 个视频，默认均为 2 个；设置保存在本机 `data/settings.json`。导出队列会在服务意外退出后自动恢复，运行中的不完整文件会清理后重新生成，不需要重新选择素材。调低并发不取消在途任务；并发越高越占用处理器和磁盘带宽，速度需以本机实际测试为准。每个转写进程采用最多 CPU 核心数的四分之一（1–8 线程），为其他任务留出资源。
